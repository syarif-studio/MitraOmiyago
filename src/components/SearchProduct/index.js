...
const SearchProduct = (props) => {
  ...
  const filteringProducts = React.useCallback(debounce(() => {

    const filtered = props.allProducts.filter(item => item.title.toLowerCase().indexOf(props.text.toLowerCase()) !== -1 )
    setSearchProduct(filtered);
    props.fetchSearchProduct(filtered);
    setIsReady(true);

  },300),[props.text, props.allProducts])
    ...
    <SafeAreaView style={{
      ...
      {isReady &&
        (searchProduct.length ?
            <ScrollView>
              <View style={{flexDirection:'row', justifyContent:'space-between', alignItems:'center', padding:7, borderBottomColor: "#eaeaea", borderBottomWidth: 1, borderBottomStyle: "solid"}}>
                <Text>Hasil Pencarian {searchProduct.length} Product</Text>
                <Button success bordered small iconRight onPress={()=>props.navigation.navigate('Search')}>
                  <Text style={{fontSize:12, color:'#009975', paddingLeft:5, paddingRight:5}}>LIHAT SEMUA</Text>
                ...
                searchProductDisplay.map((product, index) => {
                  return(
                    <TouchableOpacity onPress={() => toDetail(index)}>
                      <View style={{flexDirection:'row', justifyContent:'space-between', padding:7, borderBottomColor: "#eaeaea", borderBottomWidth: 1, borderBottomStyle: "solid"}}>
                        <Text style={{fontSize:12, flexShrink:1}}>{product.title}</Text>
                        <FontAwesomeIcon icon={faArrowRight} style={{marginTop:2, color: '#ccc'}} />
                      </View>
                    </TouchableOpacity>
                  )
                })
