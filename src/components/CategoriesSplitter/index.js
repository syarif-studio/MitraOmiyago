...
const CategoriesSplitter = (props) => {
  ...
  const handlePress = () => {
    ...
    if (categoryName.toLowerCase().includes('flash')) {
      catId = 'flashsale';
    }
  ...
  return (
    <CardItem
      style={{ backgroundColor: '#f1f1f1', marginTop: 20, marginBottom: 10 }}>
      <Body
        style={{
          flexDirection: 'row',
        ...
        {categoryName === 'flashsale' ? (
          <Text style={{ color: '#009975', fontSize: 12 }}>
            FLASH
            <Text
              style={{ fontWeight: 'bold', color: '#009975', fontSize: 12 }}>
              {' SALES'}
            </Text>
          </Text>
        ) : (
          <Text style={{ color: '#020202', fontSize: 14 }}>{categoryName}</Text>
        )}
        <Button
          onPress={handlePress}
          small
          transparent
          style={{ borderColor: '#009975', borderWidth: 2, borderRadius: 4 }}>
