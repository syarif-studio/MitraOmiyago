...
const Checkout = (props) => {
  ...
  const getUserData = async () => {
    ...
    if (userData) {
      ...
      if (cart && cart.cartAddress && cart.cartAddress.id) {
        let couriers = await checkout.getCourier(
          userData.userId,
          cart.cartAddress.id
        );
        ...
        couriers.data = couriers.data.filter(({ shipping }) => {
          return (
            shipping.status_desc === 'Digunakan' &&
            shipping.carrier_id !== 158 &&
            shipping.carrier_id !== 9
          );
        });
  ...
  const removeCoupon = async () => {
    const updateCoupon = await checkout.updateCart(
      userData.userId,
      cartVoucher.code,
      'remove_voucher'
    );
  ...
  const addCoupon = async () => {
    let updateCoupon = await checkout.updateCart(
      userData.userId,
      coupon,
      'voucher'
    );
    ...
    if (isValid) {
      ...
      updateCoupon = await checkout.updateCart(
        userData.userId,
        cartVoucher.code,
        'remove_voucher'
      );
  ...
  const handleSelectCourier = () => {
    if (couriers && couriers.totalData) {
      ActionSheet.show(
        ...
        async (buttonIndex) => {
          if (
            couriers.data[buttonIndex] &&
            couriers.data[buttonIndex].shipping.carrier_id
          ) {
            const updateCourier = await checkout.updateCart(
              userData.userId,
              couriers.data[buttonIndex].shipping.carrier_id,
              'courier'
            );
            ...
            if (updateCourier.data) {
              Toast.show({
                ...
                text:
                  updateCourier.success === 'true'
                    ? 'Kurir berhasil dipilih'
                    : updateCourier.data,
                buttonText: 'Okay',
                duration: 3000,
              });
  ...
  let courierOptions =
    couriers &&
    couriers.data &&
    Array.isArray(couriers.data) &&
    couriers.data.map(
      ({ shipping }) =>
        shipping.service_display + ' : Rp ' + priceFormat(shipping.cost)
    );
    ...
    <Container style={{ flex: 1 }}>
      <ScrollView>
        <Header
          ...
          <Body>
            <Title style={{ color: '#01906e', marginLeft: -35 }}>
              Checkout
            </Title>
          </Body>
        </Header>

        {isLoading ? null : props.cart &&
          Array.isArray(props.cart.data) &&
          userData ? (
          <Content style={{ paddingLeft: 15, paddingRight: 15 }}>
              ...
              <Card>
                <View
                  ...
                  {!isUsingCoupon && toggleCoupon ? (
                    ...
                    <Text style={{ color: '#01906e', fontSize: 15 }}>
                      Gunakan promo Omiyago
                    </Text>
                  )}
                  <Button small warning onPress={handleCoupon}>
                    {isUsingCoupon ? (
                      <Text uppercase={false}>Batal</Text>
                    ) : (
                      <Text uppercase={false}>
                        {toggleCoupon ? 'OK' : 'Kupon'}
                      </Text>
                    )}
                  </Button>
                </View>
            ...
            <View
              style={{ marginBottom: 20, paddingRight: 15, paddingLeft: 15 }}>
              <Address
                userData={userData}
                address={cartAddress}
                updateCart={handleUpdateCart}
              ...
              <View
                ...
                <Button success small onPress={handleSelectCourier}>
              ...
              <View
                ...
                <View
                  ...
                  <Text style={styles.textSub}>
                    Rp {priceFormat(props.cart.cartSubtotal)}
                  </Text>
                </View>
                <View
                  style={{
                  ...
                  <Text style={styles.textSub}>
                    Rp {priceFormat(props.cart.cartCarrier.cost)}
                  </Text>
                </View>
                <View
                  style={{
          ...
          <View>
            <Image
              ...
              source={{
                uri:
                  'https://m.omiyago.com/public/images/global/empty_product.png',
              }}
            />
            <Text
              style={{
            ...
            <Text
              style={{ fontSize: 15, textAlign: 'center', marginBottom: 10 }}>
              Daripada di kosongin keranjang mu, mending isi dengan produk
              omiyago
            </Text>
            <Button
              block
      ...
      <View
        ...
        <Text style={{ ...styles.textHeader, marginBottom: 5 }}>
          Rp {priceFormat(props.cart && props.cart.cartTotal)}
        </Text>
...
const mapDispatchToProps = (dispatch) => {
  return {
    fetchCart: (data) =>
      dispatch({ type: actionType.FETCH_CART, payload: data }),
  };
};
