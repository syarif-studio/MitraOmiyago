...
const Home = (props) => {
  ...
  const checkForUpdate = async () => {
    const res = await home.getPlatform().catch((err) => {
      console.error(err);
    });
    const platform =
      Array.isArray(res?.data) &&
      res?.data?.find((item) => item.status === 'update');
    if (
      platform.version &&
      parseFloat(platform.version) > parseFloat(config.VERSION)
    ) {
  ...
  const fetchNewProduct = async () => {
    const res = await home.getNewProduct().catch((err) => {
      console.error(err);
    });
  ...
  const fetchOmiyagoProduct = async () => {
    const res = await home.getOmiyagoProduct().catch((err) => {
      console.error(err);
    });
  ...
  const fetchPopulerProduct = async () => {
    const res = await home.getPopulerProduct().catch((err) => {
      console.error(err);
    });
  ...
  const fetchRecomendasiProduct = async () => {
    const res = await home.getRecomendasiProduct().catch((err) => {
      console.error(err);
    });
  ...
  const fetchFlashsaleProduct = async () => {
    const res = await home.getFlashSale().catch((err) => {
      console.error(err);
    });
  ...
  const fetchAllProduct = async () => {
    const res = await home.getAllProduct().catch((err) => {
      console.error(err);
    });
  ...
  const handleScroll = (e) => {
    props.navigation.setParams({
      bgColor: e.nativeEvent.contentOffset.y > 5 ? '#009975' : 'transparent',
    });
  };
  ...
  return (
    <SafeAreaView
      style={{ backgroundColor: '#fff', height: '100%', paddingTop: 5 }}>
      {loading ? (
        renderLoading()
      ) : (
        <ScrollView
          ...
          <HomeBar navigation={props.navigation} user={user} />
          <ProductCarousel />
          <View style={{ flex: 1, padding: 10, marginTop: 10 }}>
            ...
            {!!props.flashsaleProducts.length && (
              <>
                ...
                />
                <ProductItemCard
                  datas={props.flashsaleProducts.slice(0, 10)}
            ...
            <CategoriesSplitter
              categoryName={'PRODUK TERBARU'}
              navigation={props.navigation}
            />
            <ProductItemCard
              datas={props.newProducts.slice(0, 10)}
              source={'newProducts'}
              navigation={props.navigation}
            />
            <CategoriesSplitter
              categoryName={'OMIYAGO PRODUK'}
              navigation={props.navigation}
            />
            <ProductItemCard
              datas={props.omiyagoProducts.slice(0, 10)}
              source={'omiyagoProducts'}
              navigation={props.navigation}
            />
            <CategoriesSplitter
              categoryName={'PRODUK REKOMENDASI'}
              navigation={props.navigation}
            />
            <ProductItemCard
              datas={props.recomendasiProducts.slice(0, 10)}
        ...
        <View style={styless.ModalOuterContainer}>
          <View
            ...
            <View style={{ flexDirection: 'row' }}>
              ...
              <Text>
                Update Aplikasi Tersedia, Silahkan Perbarui Aplikasi anda untuk
                melanjutkan!
              </Text>
            </View>
            <TouchableOpacity
              onPress={() => {
                Linking.openURL(
                  'https://play.google.com/store/apps/details?id=com.omiyago.mobile'
                );
              }}
              style={{
                marginTop: 16,
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    fetchAllProduct: (data) =>
      dispatch({ type: actionType.FETCH_ALL_PRODUCT, payload: data }),
