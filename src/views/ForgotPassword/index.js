...
export default function ForgotPassword(props) {
    ...
    if (isEnterPassword) {
      ...
      if (checkCode?.data?.status) {
        if (checkCode?.data?.code === code) {
          const res = await login.changePassword(
            checkCode?.data?.user_id,
            text
          );
  ...
  return (
    <Container>
      <Header style={{ backgroundColor: '#FFF', marginTop: 0 }}>
        <Left>
        ...
        <Body>
          <Title style={{ color: '#333' }}>Lupa Password</Title>
        </Body>
      </Header>
      ...
      <Content>
        <View style={styles.container}>
          ...
          />
          <Text style={{ alignSelf: 'center', fontSize: 20, marginBottom: 32 }}>
            Mitra Omiyago
          </Text>
          <Form>
            {isEnterPassword && (
              <>
                <Text style={styles.TextInput}>Kode Reset</Text>
          ...
          <Button
            ...
            <Text style={styles.TextButton}>
              {isEnterPassword ? 'Reset Password' : 'Lanjut'}
            </Text>
          </Button>
          {isSucces && (
            <Button
              block
              style={styles.ButtonBlocLogin}
